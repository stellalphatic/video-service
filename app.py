...
class VideoGenerator:
    ...
    async def generate_video_wav2lip(self, image_path: str, audio_path: str, output_path: str, quality: str = "fast") -> bool:
        """Generate video using Wav2Lip - PRIORITY METHOD"""
        try:
...
async def check_models():
    ...
    if os.path.exists(sadtalker_path):
        ...
        if not missing_sadtalker:
            sadtalker_available = True
            logger.info("🎭 SadTalker models found and available")
        else:
            logger.warning(f"🎭 SadTalker missing models: {missing_sadtalker}")
    else:
    ...
    if os.path.exists(wav2lip_path):
        ...
        if not missing_wav2lip:
            wav2lip_available = True
            logger.info("👄 Wav2Lip models found and available")
        else:
            logger.warning(f"👄 Wav2Lip missing models: {missing_wav2lip}")
    else:
...
async def _run_video_generation(task_id: str, image_url: str, audio_url: str, output_dir: str, quality: str):
    """Run video generation in background with PROPER MODEL PRIORITY"""
    ...
    try:
        ...
        # PRIORITY ORDER: Try advanced models FIRST, not as fallback
        success = False
        
        # 1. FIRST PRIORITY: Try Wav2Lip if available (best for lip sync)
        if wav2lip_available:
            logger.info(f"🎤 Trying Wav2Lip FIRST for task {task_id}")
        ...
        # 2. SECOND PRIORITY: Try SadTalker if Wav2Lip failed and quality is high
        if not success and sadtalker_available and quality == "high":
        ...
        # 3. FALLBACK: Basic video only if ALL advanced models failed
        if not success:
